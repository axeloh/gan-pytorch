...
class G_AB(nn.Module):
    ...
    def __init__(self, n_filters=64):
        ...
        self.convnet = nn.Sequential(
            conv(1, n_filters, 4, padding=3),           # (b, 64, 16, 16)
            nn.LeakyReLU(0.05),
            ...
            nn.LeakyReLU(0.05),
            deconv(n_filters, 3, 4, padding=3, bn=False),  # (b, 3, 28, 28)
            nn.Tanh()
        )
...
class G_BA(nn.Module):
    ...
    def __init__(self, n_filters=64):
        super().__init__()
        self.convnet = nn.Sequential(
            conv(3, n_filters, 4, padding=3),  # (b, 64, 16, 16)
            nn.LeakyReLU(0.05),
            conv(n_filters, 2 * n_filters, 4),  # (b, 128, 8, 8)
            nn.LeakyReLU(0.05),
            conv(2 * n_filters, 2 * n_filters, 3, 1, 1),  # ("")
            ...
            deconv(2 * n_filters, n_filters, 4),  # (b, 64, 16, 16)
            nn.LeakyReLU(0.05),
            deconv(n_filters, 1, 4, padding=3, bn=False),  # (b, 1, 28, 28)
            nn.Tanh()
        )
...
class D_A(nn.Module):
    ...
    def __init__(self, n_filters=64):
        super().__init__()
        self.conv1 = conv(1, n_filters, 4, padding=3, bn=False)
        self.conv2 = conv(n_filters, 2 * n_filters, 4)
        self.conv3 = conv(2 * n_filters, 4 * n_filters, 4)

        # self.fc = conv(4*n_filters, 1, 4, 1, 0, bn=False)
        self.fc = nn.Linear(4 * 4 * (4 * n_filters), 1)
...
class D_B(nn.Module):
    ...
    def __init__(self, n_filters=64):
        super().__init__()
        self.conv1 = conv(3, n_filters, 4, padding=3, bn=False)
        self.conv2 = conv(n_filters, 2 * n_filters, 4)
